...
export function pipelineId(input: Pick<RunInput, 'pr' | 'issue' | 'description' | 'baseSha' | 'headSha' | 'runKey' | 'implementation'>, config: Config): string {
  return taskId({ version: 3, repository: config.repository, pr: input.pr?.number, base: input.baseSha, head: input.headSha,
    description: descriptionHash(input.pr, input.description), config, runKey: input.runKey, issue: input.issue, implementation: input.implementation });
}
